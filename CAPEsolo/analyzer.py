...
class Analyzer:
    ...
    def run(self):
        ...
        for module in sorted(
            Auxiliary.__subclasses__(), key=lambda x: x.start_priority, reverse=True
        ):
            ...
            try:
                ...
                log.warning(
                    "Auxiliary module %s was not implemented: %s", module.__name__, e
                )
            except Exception as e:
                log.warning(
                    "Cannot execute auxiliary module %s: %s", module.__name__, e
                )
            else:
        ...
        if not data or b":" not in data:
            ...
            if not fn:
                ...
                try:
                    ...
                    log.error(e, exc_info=True)
                    log.exception(
                        "Pipe command handler exception occurred (command %s args %s)",
                        command,
                        arguments,
                    )
