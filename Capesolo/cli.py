...
class YaraPanel(wx.Panel):
    def __init__(self, parent):
        ...
        self.filesjson = Path(ANALYSIS_DIR) / "files.json"
        self.InitUI()
...
class ConfigsPanel(wx.Panel):
    ...
    def ExtractConfigs(self, event):
        ...
        for hit in CONFIG_HITS:
            ...
            if decoderModule:
                content = f"\u2022 {hitPath}:\n\tFamily: {hitName}\n"
                if ANALYSIS_DIR not in hitPath:
                    hitPath = Path(ANALYSIS_DIR) / hitPath
...
class BehaviorPanel(wx.Panel, KeyEventHandlerMixin):
    ...
    def UpdateGenerateButtonState(self):
        logs_dir = Path(ANALYSIS_DIR) / "logs"
    ...
    def GenerateBehavior(self, event):
        ...
        behavior.set_path(ANALYSIS_DIR)
...
class StartPanel(wx.Panel):
    ...
    def GetConfig(self):
        ...
        self.debugViewPath = g_config.paths.dbgview
    ...
    def CreateAnalysisDirectory(self):
        apath = Path(ANALYSIS_DIR)
        if not apath.exists():
            apath.mkdir()

    def CopyTarget(self):
        global TARGET_FILE
        TARGET_FILE = Path(ANALYSIS_DIR) / f"s_{hash_file(hashlib.sha256, self.target)}"
    ...
    def StartAnalysis(self):
        ...
        self.analyzer = None
        try:
            self.resultserver = ResultServer("localhost", 9999, ANALYSIS_DIR)
...
class StringsPanel(wx.Panel, KeyEventHandlerMixin):
    ...
    def LoadStringsResults(self, filename):
        path = Path(ANALYSIS_DIR, filename)
        if not path.exists():
...
class DebuggerPanel(wx.Panel, KeyEventHandlerMixin):
    ...
    def PopulateLogFileDropdown(self):
        path = Path(ANALYSIS_DIR, "debugger")
        try:
    ...
    def LoadDebuggerResults(self, file_name):
        path = Path(ANALYSIS_DIR, "debugger") / file_name
        if not path.exists():
